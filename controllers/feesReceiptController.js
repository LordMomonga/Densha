...
exports.submitReceipt = async(req,res) => {
    try {
        ...
        let check1 = await StudentInfo.findOne({student_id: studentId, academic_year: accademicYear});

        if(!check1) {
            console.log('Failed check one')
            return res.status(400).send({message: "You are not a member of any school"});
        }
...
exports.schoolGetReceipts = async(req,res) => {
    try {
        ...
        let allReceipts = await FeesReceipt.find({school_id: schoolId, academic_year: accademicYear}).populate('student_id').populate('speciality').populate('account');
...
exports.schoolAcceptReceipt = async(req,res) => {
    try {
        ...
        await FeesReceipt.findOneAndUpdate({school_id: schoolId, academic_year: accademicYear, _id: receiptId},{status: 'verified'});
