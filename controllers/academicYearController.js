...
exports.createAcademicYear = async(req,res) => {
    try {
        ...
        let check = await AcademicYear.findOne({school_id: schoolId, status: 'ongoing'});
...
exports.schoolGAcademicYears = async(req,res) => {
    try {
        ...
        let allData = await AcademicYear.find({school_id: schoolId});
...
exports.stopAcademicYearStatus = async(req,res) => {
    try {
        ...
        await AcademicYear.findOneAndUpdate({school_id: schoolId, status: 'ongoing'}, {status: 'ended'});

        return res.status(200).send({message: 'Academic Year Ended'})
    } catch (error) {
        return res.status(500).send({message: "Error rejecting application"})
    }
}
